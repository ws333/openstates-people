...
export function US_generateCSVwithTitles(): void {
  ...
  const outputFile = path.join(dataDir, filenameTitleNameFax);
  ...
  try {
    ...
    const combinedRecords: TitleNameFaxRecord[] = [];
    ...
    faxRecords.forEach((faxRecord) => {
      if (faxRecord.name && faxRecord.fax) {
        ...
        combinedRecords.push({
          title: title,
          name: nameToUse, // Use full_name from legislators file when matched, fax name when not
          fax: faxRecord.fax,
        });
      }
    });
    ...
    // Log some statistics
    const senatorCount = combinedRecords.filter(
      (r) => r.title === senator
    ).length;
    const representativeCount = combinedRecords.filter(
      (r) => r.title === representative
    ).length;
    ...
    const matchedRecords = combinedRecords.filter((record) => {
      const { firstName, lastName } = normalizeName(record.name);
      const key = `${firstName}|${lastName}`;
      return legislatorMap.has(key);
    });
    console.log(
      `Records matched with legislators-current.csv: ${matchedRecords.length}`
    );
    console.log(
      `Records not matched: ${combinedRecords.length - matchedRecords.length}`
    );
